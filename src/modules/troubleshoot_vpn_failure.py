...
class TroubleshootVpnFailure:
    def __init__(self, fw_status_text, client_vpn_text, ddns, firewall_ip):
        ...
        self.ddns_address = ddns
        self.firewall_ip = firewall_ip

        """
        This method will check all of these values and show which are invalid
        User should not be able to connect if there are any tests that fail 
        (i.e grayed-out button) Each test should be clickable so that the 
        user can find out more information
        """
        # False for failed, True for passed
        self.has_passed_validation = [True] * 6
        self.run_tests()
    ...
    def test0_is_mx_online(self):
        ...
        is_online_status_code = int(
            self.fw_status_text[self.fw_status_text.find("status#") + 9])
    ...
    def test1_is_fw_reachable(self):
        ...
        if platform == 'win32':  # Identifies any form of Windows
            ...
            ping_string = "ping " + self.ddns_address
        else:  # *nix of some kind
            # ping 4 times every 200ms
            ping_string = "ping -c 5 -i 0.2 " + self.ddns_address
    ...
    def test2_is_user_behind_fw(self):
        ...
        # Start of IP position
        ip_start = self.fw_status_text.find("\"request_ip\":") + 14
        ...
        """try except clause will be more useful 
           if we know what type of error to except    
        try:
        except:
    ...
    def test3_is_client_vpn_enabled(self):
        ...
        if self.client_vpn_text[self.client_vpn_text.find(
    ...
    def test4_is_auth_type_meraki(self):
        ...
        meraki_select_type1 = self.client_vpn_text.find(
            'Meraki cloud</option></select>')
        meraki_select_type2 = self.client_vpn_text.find(
    ...
    def test5_incompatible_port_forwards(self):
        ...
        is_forwarding_500 = self.client_vpn_text.find(
            '"public_port":"500"') != -1
        is_forwarding_4500 = self.client_vpn_text.find(
            '"public_port":"4500"') != -1
        ...
        # By definition, if you can log in as a user,
        # you have a user in the Client VPN users page
    ...
    def show_results(self):
        if not self.has_passed_validation[3]:
            show_error_dialog(
                'Client VPN is not enabled in Dashboard for this network.')
